...
class ContentInvoiceProductViewModel: ContentViewModel {
    ...
    init?(contentItem: CDLineItem) {
        ...
        let atts: [NSAttributedStringKey: Any] = [
            ...
            NSAttributedStringKey.foregroundColor: UIColor.jetBlack
        ]
        
        if let discount = contentItem.product.discount {
            let linedAtts: [NSAttributedStringKey: Any] = [
                NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14),
                NSAttributedStringKey.foregroundColor: UIColor.jetBlack.withAlphaComponent(0.5),
